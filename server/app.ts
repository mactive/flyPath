...
app.get("/api/fr24/search", async (c) => {
  ...
  const payload = await memoryCache.remember(`search:${query}:${limit}`, 5 * 60_000, () =>
    provider.searchFlights(query, limit)
  );
...
app.get("/api/fr24/detail", async (c) => {
  ...
  const payload = await memoryCache.remember(`detail:${flight}:${version}`, 30_000, () =>
    provider.fetchFlightDetail(flight, version)
  );
...
if (hasBuiltClient) {
  ...
  app.get("/", (c) => {
    return c.text(
      "Frontend build not found. Run `npm run build` for production assets or use `npm run dev` during development.",
      404
    );
  });
}
