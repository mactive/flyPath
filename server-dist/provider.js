...
function deriveBoardStatusTone(icon, text) {
...
function normalizeAirportBoard(code, payload) {
...
export const provider = {
    ...
    fetchTopAirportBoards() {
        return Promise.all(DEFAULT_TOP_AIRPORT_CODES.map(async (code) => {
            const params = new URLSearchParams({ code });
            const payload = await requestJson(`${FLIGHT_VIZ_PROXY_BASE}/fr24/airport?${params.toString()}`);
